...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "persona scope"
      decision: "broad — anyone making a nontrivial decision; author-tuned tweaks allowed per decision-type when applicable, but the product addresses the general user"
    - topic: "pain shape"
      decision: "all four — decision paralysis, confirmation bias, criterion drift, missing-structure"
    - topic: "core insight"
      decision: "anti-bias as a discrete forced step (devil's advocate / pre-mortem / unknown unknowns) is the wedge — not personality, not vibe"
    - topic: "auth shape"
      decision: "passwordless email / magic link login; one account = one user's decisions"
    - topic: "role model"
      decision: "flat — every account is the same; no admin, no team, no sharing"
    - topic: "MVP scope"
      decision: "Option C — 3 weeks; real auth + persistence; 1 anti-bias mode (devil's advocate); full export (on-screen + clipboard + .md download); manual tests; manual deploy"
    - topic: "timeline"
      decision: "3-week budget; hard deadline 2026-06-18 (4 weeks from start, 1 week buffer); after-hours work"
    - topic: "anti-bias scope (re-expanded after Socrates)"
      decision: "3 user-pickable modes (devil's advocate / pre-mortem / unknown unknowns) at step 4; partial re-expansion of Option C's 1-mode cut; small effort delta (≈+2–4h)"
    - topic: "anti-bias enforcement (contradiction resolved)"
      decision: "MANDATORY — cannot skip; wedge supersedes power-user friction; Phase 1 insight + Phase 3 guardrail re-confirmed in Phase 4.5"
    - topic: "wizard interruption"
      decision: "one-shot wizard at MVP; in-memory state survives LLM error mid-wizard, but browser refresh / tab-close loses in-progress state; no DB autosave between steps"
    - topic: "saved decision mutability"
      decision: "saved artifact is read-only; no edit, no re-run wizard from saved decision in MVP (both go to Non-Goals)"
    - topic: "decisions list — delete"
      decision: "no delete at MVP; decisions are an immutable record (FR-012 removed in Socrates)"
...
Seed idea (verbatim) lives at `idea.md` in repo root. Polish-language source.
Translated and structured below as the discovery loop runs.
...
Free-form ChatGPT helps but won't force the user through structure or push back on
...
Primary persona is the **deliberate decider** — someone who recognizes a current
decision as nontrivial, knows they'll regret a gut call, and wants to *front-load*
clarity work before the research-for-specifics phase (browsing, pricing, spec-comparing).
Reached for the product mid-overload: tabs open, half-formed criteria, no artifact.

Persona is intentionally broad — programmer/rationalist (the author) and emotional
decider (estetics, values, prestige) are both in scope. The product does not judge weight
shapes; it exposes them. Author-tuned tweaks per decision-type (e.g. tech-decision prompt
phrasing) are allowed as the product grows, but the MVP must be useful to someone who is
NOT the author.
...
- The wizard never silently loses user input. If an LLM call fails mid-wizard, the
  user's typed answers from prior steps remain recoverable (not blown away by a
...
- No decision content (titles, questions, answers, artifacts) is sent to any
  third-party analytics or logging vendor. The LLM provider necessarily sees the
  content; no one else does.

## Timeline acknowledgment

Target: 3-week MVP, after-hours, ~25–30h total effort. Hard deadline 2026-06-18
(4 weeks from start; 1 week of buffer baked in). User explicitly chose the scoped-down
Option C over the 6-week full-scope Option A — the cuts (no CI/CD, no E2E, 1 of 3
anti-bias modes) are accepted as v2 work, not lost features.
...
- FR-031: User's typed answers in earlier wizard steps survive an LLM-call failure on a later step, recovered from in-memory state. Priority: must-have
...
- A user sees acknowledgement of any input within 200 ms, and continuous visible
  progress during any wizard step that depends on an LLM call (no silent waits).
- The magic-link sign-in round-trip — from the user clicking "send me a link" to
  landing on an authenticated view after clicking the link in their inbox —
  completes within 60 seconds under normal email-delivery conditions.
- The product remains usable on the latest two major versions of the four
  mainstream desktop browsers (Chrome, Firefox, Safari, Edge).

## Product framing

- **product_type**: web-app
- **target_scale.users**: small (single-digit to handful of demo testers at MVP)
- **timeline_budget.mvp_weeks**: 3
- **timeline_budget.hard_deadline**: 2026-06-18
- **timeline_budget.after_hours_only**: true

Scale probe (Socrates): at 100x scale (~1k users), LLM cost per session becomes the
dominant variable — at current Claude Sonnet 4.6 prices and ~6 LLM calls per wizard,
session cost is on the order of $0.05–$0.20, putting 1k sessions/day at roughly
$50–$200/day. "Free for users" stops being viable at that point. Out of MVP scope;
flagged for v2 as the first cost lever to address.
...
- **Live WebSearch integration** (pulling real prices / specs / reviews) — MVP
...
- **CI/CD pipeline / GitHub Actions on merge** — manual `wrangler deploy` at MVP.
  Pipeline goes to v2.
- **Playwright / E2E automated test suite** — manual smoke test at MVP. Automated
  E2E goes to v2.
...
(Informational; not part of the PRD schema. The next chain step
`/10x-tech-stack-selector` will validate these intuitions against the PRD's
product-level priors. None of these is locked here.)

Intuitions from idea.md:

- Frontend: Astro + React islands
- Auth + DB: Supabase (magic link + Postgres + RLS)
- Hosting: Cloudflare Pages (manual deploy via `wrangler` at MVP)
- LLM provider: OpenRouter with one model (Claude Sonnet 4.6 preferred, GPT-5 fallback)
- Testing: manual smoke test only at MVP; Playwright E2E goes to v2
- CI: none at MVP; GitHub Actions pipeline goes to v2

The author has constraints from a course context that may favor specific choices
in some of these slots. `/10x-tech-stack-selector` is the place that decision lives.

## Forward: technical-roadmap

(Informational; not part of the PRD schema.)

v2 candidates (in roughly the order they likely matter):

1. CI/CD pipeline (GitHub Actions → Cloudflare Pages on merge to `main`).
2. Playwright E2E happy-path test gating the pipeline.
3. Editing saved artifacts + clone-and-iterate flow.
4. Cost-per-session dashboard / LLM-call budget guardrail (precursor to scale).
5. PDF / JSON export formats alongside .md.
6. OAuth (GitHub / Google) sign-in alongside magic link.

v3+ candidates: domain packs, profile quiz, chained anti-bias (all 3 in sequence
rather than user-pickable), live WebSearch integration, decision history semantic
search, multi-user / sharing / collaborative mode, follow-up review ("3 months
later — how did it go"), voice mode, monetization.

## Quality cross-check

Run on 2026-05-21. Status: **accepted** (all gates pass; no warnings).

| Gate | Result |
| --- | --- |
| Access Control | present — magic-link login, flat user model, profile-menu sign-out; FR-001..003 + FR-032 cover the surface |
| Business Logic | present — one-sentence rule + supporting paragraphs; workflow-shaped; not empty-CRUD |
| Project artifacts | present — shape-notes.md with valid frontmatter checkpoint |
| Timeline-cost ack | present — `## Timeline acknowledgment` block records 3-week MVP target, hard deadline 2026-06-18, after-hours commitment, and explicit Option-C scope cut |
| Non-Goals | present — 8 entries (4 product-shape + 4 process) |
| Preserved behavior | n/a — greenfield session |

No items require mirroring into `/10x-prd`'s `## Open Questions`.
