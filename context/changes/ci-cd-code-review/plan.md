...
- [x] 2.5 Planted-issue PR → ai-cr:failed + naming comment; checks green (user-accepted; not live-exercised — fail-branch logic verified locally in Phase 1)
...
- [x] 2.8 Fork PR is skipped (no run, no error) (user-accepted; not exercised — no fork available; guard is in the job `if`)
